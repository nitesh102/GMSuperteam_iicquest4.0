...
const { categories, departments } = usePage().props;

const items = ref([...(categories ?? [])]);
watch(() => usePage().props.categories, (v) => {
    items.value = [...(v ?? [])];
});
...
const totalPages = computed(() =>
    Math.max(1, Math.ceil(sortedCategories.value.length / pageSize.value))
);
...
const startRecord = computed(() =>
    filteredCategories.value.length === 0 ? 0 : (currentPage.value - 1) * pageSize.value + 1
);
...
const hasCategories = computed(() => totalCategories.value > 0);
const hasSearchResults = computed(() => paginatedCategories.value.length > 0);
...
function executeDelete() {
    ...
    router.delete(route('complaint-categories.destroy', id), {
        preserveScroll: true,
    ...
    if (data.isEdit) {
        ...
        if (idx !== -1) {
            items.value[idx] = {
                ...
                department: departments.find(d => d.id === data.department_id) || items.value[idx].department,
            };
        }
    } else {
        items.value.unshift({
            ...
            description: data.description,
            department_id: data.department_id,
            department: departments.find(d => d.id === data.department_id) || null,
    ...
    <AuthenticatedLayout>
        <div class="py-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Complaint Categories</h1>
                    <p class="mt-1 text-sm text-gray-500">
                        Categorise complaints by type and assign them to departments.
                    </p>
                </div>
                <button
            ...
            <!-- STATS -->
            <div v-if="hasCategories" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl border border-gray-200 p-5 transition-all duration-200 hover:shadow-md hover:border-gray-300">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ totalCategories }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- EMPTY STATE -->
            <div v-if="!hasCategories" class="text-center py-16">
                <div class="w-20 h-20 rounded-2xl bg-indigo-50 flex items-center justify-center mx-auto mb-5">
                    <i class="fas fa-tags text-indigo-400 text-3xl"></i>
                </div>
                ...
                <p class="text-sm text-gray-500 max-w-sm mx-auto mb-6">
                    Create your first complaint category to start organizing complaints by type.
                </p>
                <button
                    @click="openCreateModal"
            ...
            <!-- TABLE CARD -->
            <div v-if="hasCategories" class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <!-- Toolbar -->
                        ...
                        <thead>
                            <tr class="border-b border-gray-100">
                                ...
                                >
                                    <span class="inline-flex items-center gap-1.5">
                                        Department
                                        <i :class="sortIcon('department') + ' text-xs'"></i>
                                    </span>
                                </th>
                                <th
                                    @click="toggleSort('description')"
                                    class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 transition-colors hidden lg:table-cell"
                                >
                                    <span class="inline-flex items-center gap-1.5">
                                        Description
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            >
                                ...
                                <td class="px-5 py-4 hidden lg:table-cell">
                                    <p class="text-sm text-gray-500 max-w-xs truncate" :title="cat.description">
                                        {{ cat.description || '—' }}
                                    </p>
                                </td>
                                <td class="px-5 py-4">
                                    <span class="text-sm text-gray-500">{{ formatDate(cat.created_at) }}</span>
                                </td>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500 mt-0.5">{{ cat.department?.name || 'No department' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <button @click="openEditModal(cat)" class="p-2 text-gray-400 hover:text-indigo-600 rounded-lg transition-colors" title="Edit">
                                    <i class="fas fa-edit text-sm"></i>
                                </button>
                                <button @click="confirmDelete(cat)" class="p-2 text-gray-400 hover:text-red-600 rounded-lg transition-colors" title="Delete">
                                    <i class="fas fa-trash text-sm"></i>
                                </button>
                            </div>
                        </div>
                        <p class="text-xs text-gray-400 mt-2 ml-11 truncate">{{ cat.description || 'No description' }}</p>
                    </div>
                </div>
                ...
                >
                    ...
                    <p class="text-sm text-gray-500 mt-1">No categories match "{{ search }}"</p>
                ...
                >
                    <p class="text-sm text-gray-500">
                        ...
                        of <span class="font-medium text-gray-700">{{ filteredCategories.length }}</span>
                    </p>
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1.5 text-sm text-gray-500 leading-relaxed">
                                    ...
                                    <span class="font-medium text-gray-700">"{{ deletingCategory?.name }}"</span>?
